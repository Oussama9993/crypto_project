...
from DES.common_func import hex2char
from RSA.RSA_enc import encrypt_rsa, generate_public_key
...
def crypto(key,message):
     # encrypt with des
    encrypted_des_hex = des_enc_exec(message, key)
...
def encrypt_data_into_image():
    ...
    data_key = key.get(1.0, "end-1c")
    data_message = txt.get(1.0, "end-1c")
    data=crypto(data_key,data_message)
    # load the image
...
key = Text(app, wrap=WORD, width=30)
key.place(x=340, y=100, height=80)
